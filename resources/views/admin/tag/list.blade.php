    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Címkék</h1>
                <p class="mt-2 text-sm text-gray-700">Itt tudja megtekinteni a hírekhez tartozó címkéket, új címkét létrehozni szerkeszteni és törölni a meglévőket.</p>
            </div>

            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                @can('Címke létrehozás')
                    <a
        ...
        <div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse ($tags as $tag)
                        <tr>
                            ...
                            <td class="flex flex-col 2xl:flex-row justify-end items-end py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                @can('Címke szerkesztés')
                                    <a href="/admin/tags/{{ $tag->id }}/edit" class="inline-flex items-center justify-center rounded-md bg-green-600 px-4 py-2 my-4 2xl:my-0 mx-0 2xl:mx-4 text-sm font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-none sm:w-auto">
                                        Szerkesztés
                                    </a>
                                @endcan
                                @can('Címke törlés')
                                    <form action="/admin/tags/{{ $tag->id }}" method="POST" class="inline-flex items-center justify-center">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-4">Nincs megjeleníthető címke</td>
                        </tr>
                    @endforelse
